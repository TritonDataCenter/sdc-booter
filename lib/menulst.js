...
const NODE_CONFIG_BOOT_PATH = fmt('/extra/joysetup/%s', NODE_CONFIG_FNAME);
...
function buildMenuLst(opts, cb) {
    ...
    if (opts.os === 'linux') {
        ...
        var serial = 'ttyS' + console_config.serial_unit;

        result.push(
            fmt('variable os_console console=%s,115200n8', serial),
            '');

        result.push(
            'title Live 64-bit',
            fmt('   kernel$ %s console=tty0 console=%s,115200n8',
                fmt(LINUX_KERNEL_FMT, c.platform),
                serial),
        ...
        result.push(fmt('  module$ %s type=file name=etc/%s',
...
function buildIpxeCfg(opts, cb) {
    ...
    var kargs = (os === 'smartos') ? kargs_arr.join(',') : kargs_arr.join(' ');
    var module = fmt(
    ...
    if (os === 'smartos') {
        ...
        const serial = 'ttyS' + console_config.serial_unit;
        result.push(
            fmt('kernel %s boot=live console=tty0 console=%s,115200n8 ' +
                'BOOTIF=01-%s ip=%s:::%s:::none %s fetch=%s://%s%s',
                kernel, serial, mac_dashed, c.ip, c.netmask,
                kargs, proto, opts.serverIp, squashfs));
        ...
        if (use_hash) {
            result.push(fmt('module %s://%s%s.hash %s.hash',
                proto, opts.serverIp, squashfs, path.basename(squashfs)));
            result.push(fmt('module %s://%s%s.hash %s.hash',
                proto, opts.serverIp, module, path.basename(module)));
        }

        result.push(fmt('module %s://%s%s /etc/%s',
            proto, opts.serverIp, NODE_CONFIG_BOOT_PATH,
            NODE_CONFIG_FNAME));
    }
