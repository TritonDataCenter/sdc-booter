...
function buildIpxeCfg(opts, cb) {
    ...
    if (os === 'smartos') {
        ...
        result.push(
            fmt('kernel %s boot=live console=ttyS0 fetch=%s://{next-server}%s',
                kernel, proto, squashfs));
        ...
        if (use_hash) {
            result.push(
                fmt('module --name %s.hash fetch=%s://{next-server}%s.hash' +
                    ' type=hash', squashfs, proto, squashfs));
