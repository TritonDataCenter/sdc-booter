...
function buildIpxeCfg(opts, cb) {
    ...
    if (os === 'smartos') {
        ...
        result.push(
            fmt('kernel %s boot=live console=ttyS0 BOOTIF=01-%s ip=%s:::%s:: ' +
                'fetch=%s://%s%s', kernel, mac_dashed, c.ip, c.netmask, proto,
                opts.serverIp, squashfs));

        result.push(fmt('initrd %s name=ramdisk', module));
        // Not yet:
        ...
        if (use_hash) {
            result.push(
                fmt('module --name %s.hash fetch=%s://%s%s.hash' +
                    ' type=hash', squashfs, proto, opts.serverIp, squashfs));

            result.push(fmt(
                'module --name %s.hash type=hash name=ramdisk', module));
        }
    }
    ...
    if (!opts.disableBootTimeFiles && opts.bootFiles &&
        ...
        opts.bootFiles.forEach(function (bf) {
            ...
            if (os === 'smartos') {
                ...
                result.push(fmt(
                    'module --name %s %s://${next-server}%s/%s type=%s ',
                    fname, proto, opts.bootFsDirRelative, bf, ftype));
            }
        });
    }
