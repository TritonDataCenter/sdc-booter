...
var DHCPPacket = function(op, htype, hlen, hops, xid, secs, flags, ciaddr, yiaddr, siaddr, giaddr, chaddr, sname, file, options) {
    this.op      = op;
    this.htype   = htype   || 1;
    this.hlen    = hlen    || 6;
    this.hops    = hops    || 0;
...
DHCPPacket.parse = function(data) {
    ...
    res = pack.unpack("CCCCNnnNNNNC16a64a128CCCCC*", data.toString('binary'));
    ...
    pkt_sname  = res[27];    // a64
    pkt_file   = res[28];    // a128
    ...
    while (i < res.length) {
        ...
        if (option == 0) {
            ...
            olen = res[i++]; // length of option data
            value=[];
    ...
    return(new DHCPPacket(
            pkt_op, pkt_htype, pkt_hlen, pkt_hops, pkt_xid, pkt_secs, pkt_flags
          , pkt_ciaddr, pkt_yiaddr, pkt_siaddr, pkt_giaddr, pkt_chaddr
...
DHCPPacket.prototype.raw = function() {
    ...
    for(i=0; i<9; i++) {
        options.write(pack.pack("C", parseInt("00", 16)), opt_len++, 'binary');
    }
            ...
            for (opt in data['options']) {
                ...
                switch(parseInt(opt, 10)) {
                    ...
                    case 12:  // hostname
                        pkt.options[opt] = []
                        for(var i=0; i<value.length; i++) {
                            pkt.options[opt].push(value.charCodeAt(i));
                        }
                        break;
                    // 4 byte integers
                    case 51:  // lease time
                        pkt.options[opt] = pack.unpack("CCCC", pack.pack("N", parseInt(value, 10)));
                        break;
                    default:
                        throw("Ignoring unhandled option "+opt);
                }

                if (pkt.options[opt] != undefined) {
                    var arg_array = [];
                    var fmt="";
                    for (var i=0; i<pkt.options[opt].length; i++) {
                        arg_array.push(pkt.options[opt][i]);
                        fmt="%02x "+fmt;
                    }
