...
DHCPPacket.prototype.raw = function () {
    ...
    for (var option in this.options) {
        if ((option > 0) && (option < 255)) {
            ...
            opt_len += options.write(pack.pack('CC', parseInt(option, 10),
                value.length), opt_len, 'binary');
            for (var i = 0; i < value.length; i++) {
                opt_len += options.write(pack.pack('C', value[i]), opt_len,
                  'binary');
            }
        }
    }
