...
function getBootParams(opts, callback) {
    ...
    vasync.pipeline({
        ...
        funcs: [
            ...
            function _getNic(fArg, cb) {
                napi.getNic(mac, function (err, res) {
                    ...
                    // If we have the cn uuid now, set it for napiGetAggrs
                    fArg.cn_uuid = fArg.bootNic.belongs_to_uuid;
                    cb();
                });
            },

            // Get all the aggrs for this CN (if setup). If the CN is not
            // setup, this acts like the goggles (do nothing).
            mod_clients.napiGetAggrs,

            // Check if 'mac' is in one of the aggrs on the CN. If so,
            // set fArg.aggr_mac to the first mac of the aggr.
            function _checkAggr(fArg, cb) {
                fArg.aggr_id = null;
                fArg.aggr_mac = null;

                // If no aggrs on the CN, nothing to do
                if (fArg.aggrs.length === 0) {
                    cb();
                    return;
                }

                opts.aggrs.forEach(function eachAggr(a) {
                    a.macs.forEach(function eachMac(m) {
                        if (m === mac) {
                            fArg.aggr_id = a.id;
                            fArg.aggr_mac = a.macs[0];
                            log.debug('NIC "%s" is in aggregation %s (%s), ' +
                                'aggr MAC is "%s"',
                                mac, a.name, a.id, a.macs[0]);
                        }
                    });
                });
            },

            // If 'mac' is in an aggr, get the IP of the aggr
            function _getAggrIp(fArg, cb) {
                fArg.aggr_ip = null;

                // If not in an aggr, skip
                if (fArg.aggr_id === null) {
                    cb();
                    return;
                }

                // If mac is the first mac of the aggr, we can use the
                // existing ip we have (if we have any). If the aggr
                // doesn't have an IP provisioned, we'll do that instead
                // (a bit further below).
                if (mac === fArg.aggr_mac) {
                    if (fArg.bootNic && fArg.bootNic.ip) {
                        fArg.aggr_ip = fArg.bootNic.ip;
                    }
                    cb();
                    return;
                }

                napi.getNic(fArg.aggr_mac, function onMac(err, res) {
                    if (err) {
                        // Unlike earlier, if 'mac' is in an aggr, then
                        // 'mac' should already exist in NAPI. If it does
                        // not, something's gone awry
                        log.error(err, 'Error getting aggr nic "%s" from NAPI',
                            mac);
                        cb(err);
                        return;
                    }
            ...
            // If the nic is in an aggr, and the aggr doesn't have an IP,
            // provision an IP for the aggr.
            function _provisionIP(fArg, cb) {
                if (fArg.bootNic === null || fArg.bootNic.ip || fArg.aggr_ip) {
                    cb();
                    return;
                }

                // If 'mac' is in an aggr, we should have the aggr id (uuid)
                // already as well as the mac for the aggr itself.
                var type = 'nic';
                var provisionMac = mac;

                if (fArg.aggr_id) {
                    type = 'aggr nic';
                    provisionMac = fArg.aggr_mac;
                }
                ...
                log.debug(putParams, 'Updating %s "%s" to add IP', type,
                    provisionMac);
                napi.updateNic(provisionMac, putParams,
                    function onNic(err, res) {
                    if (err) {
                        log.error({err: err, params: putParams},
                            'Error adding IP to %s "%s" on NAPI', type,
                            provisionMac);
                        cb(err);
                        return;
                    }

                    log.debug(res, 'Updated %s "%s" with IP "%s" in NAPI',
                        type, provisionMac, res.ip);
            ...
            function _createNic(fArg, cb) {
                // If the NIC exists or in an aggr, skip. For the aggr case,
                // if it is in an aggr, the NIC must already exist in
                // NAPI
                if (fArg.bootNic !== null || fArg.aggr_id) {
                    cb();
                    return;
                }
            ...
            function _getUnsetupServerUUID(fArg, cb) {
                // We tried to set this earlier, but if the CN isn't setup
                // (and thus any of its NICs cannot yet be in an aggr),
                // then we weren't able to assign cn_uuid earlier, so
                // we do it now.
                fArg.cn_uuid = fArg.bootNic.belongs_to_uuid;
        ...
        // XXX: if we're trying to boot of a NIC in an aggr, we probably
        // need to modify this, but it's not yet clear to me how all of this
        // is used and interacts with the rest of booter to understand
        // how to modify it -- including things like 'admin_nic' and such.
        if (pipelineTimedOut) {
