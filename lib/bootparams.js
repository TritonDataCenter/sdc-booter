...
function getBootParams(options, callback) {
    ...
    vasync.pipeline({
        ...
        params.kernel_args.admin_ip = bootNic.ip;
        params.netmask = bootNic.netmask;
        params.kernel_args.admin_netmask = bootNic.netmask;
        ...
        for (var n in nics) {
            ...
            for (var t in nic.nic_tags_provided) {
                ...
                if (nic.nic_tags_provided[t] === 'admin' &&
                    nic.hasOwnProperty('ip') &&
                    nic.hasOwnProperty('netmask')) {

                    params.kernel_args.admin_ip = nic.ip;
                    params.kernel_args.admin_netmask = nic.netmask;
                }
            }
            seen[newMAC] = 1;
        }
