...
function getBootParams(opts, callback) {
    ...
    vasync.pipeline({
        ...
        funcs: [
            ...
            function _getNodeConfig(fArg, cb) {
                mod_node_config.getNodeConfig(opts, function getCfgCb(getErr) {
                    if (getErr) {
                        log.error(getErr, util.format(
                            'Error getting %s file', mod_node_config.fileName));
                        cb(getErr);
                        return;
                    }
                    fArg.nodeconfig = util.format('%s/%s/%s', opts.tftpRoot,
                        mod_node_config.dirName, mod_node_config.fileName);
                    cb();
                });
            }
        ]
