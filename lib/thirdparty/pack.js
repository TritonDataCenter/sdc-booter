...
/*
From: node-mysql -- A node.js interface for MySQL
...
exports.unpack = function(format, data) {
    ...
    while(format) {
        ...
        case 'n': // unsigned short (always 16 bit, network / big endian byte order)
            if (quantifier === '*') {
                quantifier = (data.length) / 2;
            } else {
                quantifier = parseInt(quantifier, 10);
            ...
            data = data.slice(quantifier*2);
            for (i=0;i<currentData.length;i+=2) {
                currentResult = parseInt(currentData.charCodeAt(i+1)) +
                    parseInt(currentData.charCodeAt(i) << 8);
                result.push(currentResult);
            }
            break;

        case 'N': // unsigned long (always 32 bit, network / big endian byte order)
            if (quantifier === '*') {
                quantifier = (data.length) / 4;
            } else {
                quantifier = parseInt(quantifier, 10);
            ...
            for (i=0;i<currentData.length;i+=4) {
                currentResult =
                    parseInt((currentData.charCodeAt(i+3) & 0xFF)) +
                    parseInt((currentData.charCodeAt(i+2) & 0xFF) << 8) +
                    parseInt((currentData.charCodeAt(i+1) & 0xFF) << 16) +
                    parseInt((currentData.charCodeAt(i)   & 0xFF) << 24);
                result.push(currentResult);
            }
            break;
...
exports.pack = function(format) {
    ...
    while(format) {
        ...
        case 'n': // 16-bit network / big endian byte order
            if (quantifier === '*') {
                quantifier = args.length - argumentPointer;
            }
            if (quantifier > (args.length - argumentPointer)) {
