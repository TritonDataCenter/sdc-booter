...
function DHCPD(opts) {
  ...
  this.sock = dgram.createSocket("udp4",
    function (msg, peer) { self.handleMessage(msg, peer); });
...
DHCPD.prototype.start = function() {
  ...
  this.sock.on('listening', function() {
      ...
      self.log.info("dhcpd started on %s:%d", self.config.listenIp, self.config.port);
  });
...
DHCPD.prototype.handleMessage = function(msg, peer) {
    ...
    if (log.debug()) {
        for(var i = 0; i < msg.length; i += 4) {
          // XXX
          this.log.debug(sprintf("[%03d]: 0x%02x 0x%02x 0x%02x 0x%02x", i, msg[i], msg[i+1], msg[i+2], msg[i+3]));
        }
    }

    packet.dump(function (msg) {
      log.info(msg);
    });
      ...
      'funcs': [
        ...
        function _params(arg, cb) {
          bootparams.getBootParams(
            mac, self.napi, self.cnapi, log, function(err, res) {
              if (err) {
                return cb(err);
              }
              params = res;
        ...
        function _menulst(arg, cb) {
          ...
          menulst.writeMenuLst(
            mac, params, self.config.tftpRoot, log, function(err) {
              if (err) {
                return cb(err);
              }
 ...
 */
DHCPD.prototype.sendReply = function(packet, packetOpts, params, log, callback) {
  var mac = packet.chaddr;
  var port = 68;
...
DHCPD.prototype.buildResponse = function(packet, log) {
    ...
    if (packet.options[77]) {
        ...
        if (userClass === 'gPXE') {
            ...
            log.info("Detected gPXE, setting file to '%s'",
                packetOpts.file);
        } else {
