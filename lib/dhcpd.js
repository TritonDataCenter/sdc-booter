...
DHCPD.prototype.handleMessage = function (msg, peer) {
    ...
    'funcs': [
        ...
        function _menulst(arg, cb) {
            packetOpts = self.buildPacketOpts(packet, params, log);
        ...
        function _reply(arg, cb) {
            self.sendReply(packet, packetOpts, log, cb);
        }

    ] }, function (err, res) {
        if (err) {
            log.error(err, 'Error creating reply');
        } else {
            log.info('Reply sent');
        }
    });
...
function createServer(opts) {
    ...
    var requiredStrings = [ 'listenIp', 'tftpRoot', 'defaultGateway',
        'serverIp', 'leaseTime', 'netmask'];
